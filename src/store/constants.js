export const SET_WORD_COUNT = 'SET_WORD_COUNT'
export const SET_DOC_INFO = 'SET_DOC_INFO'
export const UPDATE_TEXT = 'UPDATE_TEXT'
export const UPDATE_READ_COUNT = 'UPDATE_READ_COUNT'
export const SET_PUB_KEY = 'SET_PUB_KEY'
export const SET_AES_KEY = 'SET_AES_KEY'
export const SET_USER = 'SET_USER'
export const SET_DOCUMENT_LIST = 'SET_DOCUMENT_LIST'
export const SET_CURRENT_DOC = 'SET_CURRENT_DOC'
export const CLEAR_CURRENT_DOC = 'CLEAR_CURRENT_DOC'
export const SET_CURRENT_CONTENT = 'SET_CURRENT_CONTENT'
export const SET_MODAL_VISIBILITY = 'SET_MODAL_VISIBILITY'
export const INITIALIZE_MODAL = 'INITIALIZE_MODAL'
export const SET_INVOICE = 'SET_INVOICE'
export const CHANGE_SECONDS = 'CHANGE_SECONDS'
export const SET_RATE = 'SET_RATE'
export const SET_STATUS = 'SET_STATUS'
export const SET_STATUS_PAID = 'SET_STATUS_PAID'
export const SET_MACAROON = 'SET_MACAROON'
export const SET_NODE_INFO = 'SET_NODE_INFO'
export const UPDATE_DOCUMENT = 'UPDATE_DOCUMENT'
export const SET_DOCS_LOADING = 'SET_DOCS_LOADING'
export const SET_CURRENT_LIKES = 'SET_CURRENT_LIKES'
export const SET_CURRENT_DISLIKES = 'SET_CURRENT_DISLIKES'
export const SET_BOLTWALL_URI = 'SET_BOLTWALL_URI'
